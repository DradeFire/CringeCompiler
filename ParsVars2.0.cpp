...
public:
    ...
    string typeWar = "int";//для проверки ParsVars
    ...
    string Check_Global_Vars(string word, string typeVar) {
        ...
        if (typeVar == "short") {
            auto it = this->globalVarShort.find(word);
            if (it != this->globalVarShort.end()) {
                word = to_string(it->second);
                return word;
            };
        };
        if (typeVar == "char") {
            auto it = this->globalVarChar.find(word);
            if (it != this->globalVarChar.end()) {
                word = to_string(it->second);
                return word;
            };
        };
                        ...
                        if (word == "=") {
                            ...
                            if (semicolon != -1) {
                                if (word.length() == semicolon + 1) {
                                    ...
                                    endCommand = ";";
                                    if (typeVar == "int") {
                                        if (Check_Global_Vars(word, typeVar) != "non_have_var") { word = Check_Global_Vars(word, typeVar); }
                                        if (Check_Local_Vars(word, typeVar) != "non_have_var") { word = Check_Local_Vars(word, typeVar); }
                                    ...
                                    if (typeVar == "char") {
                                        if ((word[0] == '\'') && (word[word.size() - 1] == '\'')) {
                                            ...
                                            if (Check_Global_Vars(word, typeVar) != "non_have_var") { word = Check_Global_Vars(word, typeVar); }
                                            if (Check_Local_Vars(word, typeVar) != "non_have_var") { word = Check_Local_Vars(word, typeVar); }
                                            char_value = stoi(word); writing_arguments(nameVar, char_value, typeVar);
                                        };
                                    };
                                }
                                else { throw exception(); };
